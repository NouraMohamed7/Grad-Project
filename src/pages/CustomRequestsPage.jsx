...
export default function CustomRequestOrdersPage() {
  ...
  // ✅ FIXED: Now fetches from API with pagination
  const fetchOrders = async (p = 1) => {
    ...
    try {
      ...
      //   custom_request: { doctor: { all_user: { email, fullname } } } }], last_page, per_page, total }
      const raw = res?.data ?? [];
  ...
  const handleStatusChange = async (order, newStatus) => {
    ...
    try {
      ...
      if (res.success !== false) {
        setOrders((prev) =>
          prev.map((o) =>
            o._rowKey === order._rowKey ? { ...o, status: newStatus } : o,
          ),
        );
      } else {
        alert(
  ...
  // Cancel is UI-only for now — no backend endpoint exists yet
  const handleLocalCancel = (order) => {
    setOrders((prev) =>
      prev.map((o) =>
        o._rowKey === order._rowKey ? { ...o, status: "cancelled" } : o,
      ),
    );
  };
  ...
  // ✅ FIXED: Cleaned up - only using actual API path
  ...
  // Note: For server-side search, you'd need a search param in the API
  ...
  // ✅ FIXED: Pagination now comes from API (lastPage), not calculated locally
                ...
                filtered.map((order) => {
                  ...
                  return (
                    <tr
                      ...
                      onClick={() =>
                        navigate(
                          `/requests/order-details/${order.id ?? order._rowKey}`,
                        )
                      }
                    >
                      ...
                      <td onClick={(e) => e.stopPropagation()}>
                        ...
                        >
                          <select
                            value={status}
                            disabled={
                              updatingId === order.id || status === "cancelled"
                            }
                            onChange={(e) =>
                          ...
                          >
                            ...
                            {/* keep current value visible even if it's outside the editable list */}
                          ...
                          {status !== "cancelled" && (
                            <button
                              type="button"
                              onClick={() => handleLocalCancel(order)}
                              title="Cancel (UI only — no backend endpoint yet)"
                              style={{
                                border: "none",
